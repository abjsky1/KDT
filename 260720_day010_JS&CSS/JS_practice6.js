...
function renderProducts(List){
    for(i=0 ; i<=List.length-1 ; i++){
        html1 += `<div> <h4>${List[i].name}</h4> <p>${List[i].price}</p> <p>${List[i].stock}</p> </div>`
    }
}

renderProducts(products)
document.querySelector('p6').innerHTML = html1
...
document.querySelector('p8').innerHTML = html2
