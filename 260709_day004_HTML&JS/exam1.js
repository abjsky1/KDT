...
// 1. if문 : if( 조건식 ) 실행문
// 조건식 결과가 true(참) 이면 실행문 처리
if(10>3) console.log( `1. 10은 3보다 크다.`)    // 실행 O
if(10>30) console.log( `1. 10은 30보다 크다.`)  // 실행 X

// 2. if문 : if( 조건식 ){ 실행문; 실행문; }
// 실행문이 2개 이상이면 { } 로 묶는다
if(10>3){ 
...
// 4. if문 ~ else if ~ else if ~ else :
// 다수 조건에 따른 true/false 결과를 1개만 처리
...
let 점수2 = Number(prompt(`두번째 점수 입력`))
let 총점 = 점수1 + 점수2
...
else if(정수1 > 정수2){console.log(`${정수2}`)}
...
let 비밀번호 = prompt(`비밀번호 입력`)
let 비밀번호길이 = 비밀번호.length
if(비밀번호길이 < 8){console.log(`보안 등급: 약함 (8자 이상으로 설정해주세요.)`)}
else if(비밀번호길이 < 12){console.log(`보안 등급: 보통`)}
...
let 비밀번호 = prompt(`비밀번호 입력`)
let 비밀번호길이 = 비밀번호.length
if(비밀번호길이 >= 12){console.log(`보안 등급: 강함`)}
else if(비밀번호길이 >= 8){console.log(`보안 등급: 보통`)}
...
let seatList = [ 'O', 'X', 'O' ]
let indexNumber = parseInt(prompt(`예약할 좌석 번호(0~2) 입력`))
if(seatList[indexNumber] == `O`){console.log(`예약 불가`)}
...
let 게임점수 = parseInt(prompt(``))
if(게임점수 >= 900){console.log(`A`)}
else if(게임점수 >= 700){console.log(`B`)}
else if(게임점수 >= 500){console.log(`C`)}
...
해당하는 음료 이름과 가격을 
'선택하신 음료는 [음료이름]입니다. 가격은 [가격]원입니다.' 형식으로 출력하세요. 
만약 목록에 없는 번호를 입력하면 
'없는 상품입니다.'라고 출력하는 프로그램을 작성하시오. */
...
let drinkNumber = parseInt(prompt(`'콜라 : 0', '사이다 : 1', '커피 : 2'`))
